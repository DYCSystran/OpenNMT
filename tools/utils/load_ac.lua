...
ffi.cdef[[
  void* ac_create(const char** str_v, unsigned int* strlen_v,
                  unsigned int v_len);
  int ac_match(void*, const char *str, int len);
  int ac_match2(void*, const char *str, int len);
  int ac_match_idx(void*, const char *str, int len);
  int* ac_match_idx_all(void*, const char *str, int len);
  void ac_free(void*);
]]
...
function _M.load_ac_lib()
    if ac_lib ~= nil then
        ...
        if so_path ~= nil then
            ...
            ac_match = ac_lib.ac_match_idx_all
...
function _M.match(ac, str)
    local r = ffi.new("const int *[?]", 1000)
    r = ac_match(ac, str, #str);
    local matches = {}
    for i = 0, 1000 do
        if r[i] ~= 0 then
          table.insert(matches, r[i])
        else
          return matches
        end
    end
